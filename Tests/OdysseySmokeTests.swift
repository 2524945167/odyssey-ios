...
final class OdysseySmokeTests: XCTestCase {
    ...
    // 3. canTranslate 必须把空格、换行和制表符视为空内容
    @MainActor
    ...
    // 7. 复制功能通过轻量 ClipboardWriting 注入，测试不修改真实系统剪贴板
    @MainActor
    ...
    // 8. performMockTranslation 是可等待的 async 方法，阻止重复触发，defer 恢复 isTranslating
    @MainActor
    ...
    // 9. ContentView 在 393 × 852 浅色模式下可通过 ImageRenderer 渲染
    @MainActor
