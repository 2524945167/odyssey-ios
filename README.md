...
**第 8.1 轮设置简化与通用思考适配已通过 CI，等待真机验收**。[Actions 第 38 次运行](https://github.com/2524945167/odyssey-ios/actions/runs/34104318725) 的 181 项测试全部通过，编译与 Actions 警告为 0，12 张浅深色截图及未签名 IPA 下载、结构和哈希检查完成。风格选择已移至“设置 → 翻译风格”，“网络游戏聊天”改为“网络聊天”，保留用户编辑的旧模板。思考设置按已核实的官方端点、协议和模型简洁适配；未知或需特殊定制的配置跟随模型默认，不猜测参数。未调用真实模型，不声称所有模型兼容或已实测提速。范围、官方依据、模拟器诊断和产物证据见 [第 8.1 轮验收记录](ROUND8_1_VERIFICATION.md)。以下第 8 轮描述为此次修改前的历史记录。
