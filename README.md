...
**第 7 轮真实流式翻译接入已在本地实现，等待 CI 与真机验收**。正式翻译默认输出上限 8192 tokens、无数据超时 60 秒，两项可在“设置 → 翻译参数”自定义；单次请求最多 5 分钟。连接测试的 256 tokens / 30 秒保持独立。范围与待验收项见 [第 7 轮验收记录](ROUND7_VERIFICATION.md)。尚未执行真实模型调用。
