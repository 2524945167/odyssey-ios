...
**第 8.1 轮设置简化与通用思考适配待 CI 验证**。风格选择已移至“设置 → 翻译风格”，“网络游戏聊天”改为“网络聊天”，保留用户编辑的旧模板。思考设置按已核实的官方端点、协议和模型简洁适配；未知或需特殊定制的配置跟随模型默认，不猜测参数。未调用真实模型，尚未真机验收。范围与依据见 [第 8.1 轮验收记录](ROUND8_1_VERIFICATION.md)。以下第 8 轮描述为此次修改前的历史记录。
